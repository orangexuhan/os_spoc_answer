...
```c
//funcall.c line 24
ret = write(1, "2013011355, 2013011339", 24);
```
```txt
...
00000000  0000080e  LL    0x8 (D 8)    // 向a寄存器中存8
root/usr/funcall.c  6:   asm(LBL,16); // load register b with val
00000004  00001026  LBL   0x10 (D 16)    // 向b寄存器中存16
root/usr/funcall.c  7:   asm(BOUT);   // output byte to console
00000008  0000009a  BOUT         // 输出
root/usr/funcall.c  8: }
root/usr/funcall.c  9: 
root/usr/funcall.c  10: int write(int f, char *s, int n)
0000000c  00000002  LEV   0x0 (D 0)   // 改变pc值
root/usr/funcall.c  11: {
root/usr/funcall.c  12:   int i;
root/usr/funcall.c  13:   ret = 1;
00000010  fffff801  ENT   0xfffffff8 (D -8)  // return，sp-8
00000014  00000123  LI    0x1 (D 1)		// 赋给a常数值
00000018  00000045  SG    0x0 (D 0)		// 将寄存器a值存在内存
root/usr/funcall.c  14:   i=n;
0000001c  0000200e  LL    0x20 (D 32)		// 更改寄存器a的值
00000020  00000440  SL    0x4 (D 4)		// 将寄存器a的值存在内存
root/usr/funcall.c  15:   while (i--)
00000024  00000003  JMP   <fwd>			// 跳转，更改pc
root/usr/funcall.c  16:     out(f, *s++);
00000028  0000180e  LL    0x18 (D 24)		// 更改寄存器a的值
0000002c  ffffff57  SUBI  0xffffffff (D -1)
00000030  00001840  SL    0x18 (D 24)		// 将寄存器a的值存在内存
00000034  ffffff1f  LXC   0xffffffff (D -1)	// 读取内存值 存在寄存器a
00000038  0000009d  PSHA			// 寄存器a入栈
0000003c  0000180e  LL    0x18 (D 24)
00000040  0000009d  PSHA
00000044  ffffb805  JSR   0xffffffb8 (TO 0x0)	// 存储当前pc
00000048  00001001  ENT   0x10 (D 16)
root/usr/funcall.c  17:   return i;
0000004c  0000040e  LL    0x4 (D 4)
00000050  00000157  SUBI  0x1 (D 1)		// 算术运算
00000054  00000440  SL    0x4 (D 4)
00000058  00000154  ADDI  0x1 (D 1)		// 算术运算
0000005c  00000086  BNZ   <fwd>			// 条件跳转
...
root/usr/funcall.c  24:   ret = write(1, "2013011339 2013011355",24);
0000006c  0000189e  PSHI  0x18 (D 24)		// 操作数入栈
00000070  00000008  LEAG  0x0 (D 0)		// a=pc+operand0
00000074  0000009d  PSHA
00000078  0000019e  PSHI  0x1 (D 1)
0000007c  ffff9005  JSR   0xffffff90 (TO 0x10)	// 当前pc入栈
...
- funcall中的全局变量ret放在内存中何处？如何对它寻址？
	- 00000084  00000045  SG    0x0 (D 0) 存放在0xb8处，相对于PC寻址
- funcall中的字符串放在内存中何处？如何对它寻址？
	- 00000070  00000008  LEAG  0x0 (D 0) 存放在0xa0处，相对于PC寻址
- 局部变量i在内存中的何处？如何对它寻址？
	- sp+4，相对于sp寻址
- 当前系统是处于中断使能状态吗？
	- 不在
