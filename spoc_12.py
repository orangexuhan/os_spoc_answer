...
class scheduler:
    ...
    def next_proc(self, pid=-1):
        if pid == -1:
            pid = self.curr_proc
        for i in range(1, (self.get_num_processes()) + 1, 1):
            if self.proc_info[(pid + i) % (self.get_num_processes())][PROC_STATE] == STATE_READY:
                self.curr_proc = (pid + i) % (self.get_num_processes())
